...
if (isset($_GET['edit_id'])) {
    ...
    $get_data_query = "SELECT * FROM `user_orders` WHERE order_id = ?";
...
if (isset($_POST['update_order_status'])) {
    ...
    if (empty($order_status)) {
        $error_message = "Please select an order status.";
    } else {
        $update_query = "UPDATE `user_orders` SET order_status = ? WHERE order_id = ?";
        $stmt = mysqli_prepare($con, $update_query);
        mysqli_stmt_bind_param($stmt, "si", $order_status, $edit_id);

        if (mysqli_stmt_execute($stmt)) {
            $success_message = "Order status updated successfully.";
        } else {
            $error_message = "Error updating order status: " . mysqli_error($con);
        }
        mysqli_stmt_close($stmt);
    }
}
...
<head>
    ...
    <title>Edit Order Status - Admin Dashboard</title>
